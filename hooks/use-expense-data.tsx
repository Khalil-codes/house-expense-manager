...
export function useExpenseData() {
  const [data, setData] = useLocalStorage<ExpenseData>("houseExpenseData", {
    construction: [],
    property: [],
    loan: {
      amount: 0,
  ...
  // Convert string dates to Date objects after retrieving from localStorage
  useEffect(() => {
    setIsLoading(true);

    // Convert date strings back to Date objects
    if (data.construction) {
      data.construction.forEach((expense: any) => {
    ...
    if (data.property) {
      data.property.forEach((expense: any) => {
        expense.date = new Date(expense.date);
        // Add missing fields for backward compatibility
        if (expense.paidTo === undefined) expense.paidTo = "";
        if (expense.coveredByLoan === undefined) expense.coveredByLoan = false;
    ...
    if (data.loan && data.loan.payments) {
      data.loan.payments.forEach((payment: any) => {
        payment.date = new Date(payment.date);
      });
    }

    if (data.loan && data.loan.startDate) {
      data.loan.startDate = new Date(data.loan.startDate);
    }

    if (data.loan && data.loan.prepayments) {
      data.loan.prepayments.forEach((prepayment: any) => {
  ...
  const editExpense = (
    ...
    setData((prevData) => ({
      ...prevData,
      [type]: prevData[type].map((expense) =>
  ...
  const getGrandTotal = () => {
    ...
    const loanPaidTotal = data.loan.payments.reduce(
    ...
    try {
      ...
      if (parsedData.construction) {
        parsedData.construction.forEach((expense: any) => {
          ...
          // Add missing fields for backward compatibility
          if (expense.paidTo === undefined) expense.paidTo = "";
          if (expense.coveredByLoan === undefined)
            expense.coveredByLoan = false;
        });
      }
